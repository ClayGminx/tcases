...
import static org.cornutum.tcases.openapi.OpenApiUtils.resolveSchema;
import static org.cornutum.tcases.openapi.OpenApiUtils.resolveSchemaMembers;
...
public class SchemaAnalyzer extends ContextHandler<OpenApiContext> {
                            ...
                            .map(
                                    notSchema ->
                                            assertNot(
                                                    ...
                                                    (Schema) notSchema,
                                                    (s, v) -> {
                                                        s.setProperties(MapBuilder.of(property, v).build());
    ...
    private List<Schema<?>> getNumericInvalidators(Schema<?> schema) {
        ...
        Optional.ofNullable(schema.getMinimum())
                ...
                .ifPresent(s -> {
                    s.setExclusiveMaximum(exclusiveMaximum);
                    alternatives.add(s);
                });
        ...
        Optional.ofNullable(schema.getMaximum())
                ...
                .ifPresent(s -> {
                    s.setExclusiveMinimum(exclusiveMinimum);
                    alternatives.add(s);
                });
